...
class BuildingNode: SKSpriteNode {
    ...
    func drawBuilding(size: CGSize) -> UIImage {
        ...
        let img = renderer.image { ctx in
            ...
            ctx.cgContext.setFillColor(colour.cgColor)
            ctx.cgContext.addRect(rectangle)
            ctx.cgContext.drawPath(using: .fill)
            
            let lightOnColour = UIColor(hue: 0.190, saturation: 0.67, brightness: 0.99, alpha: 1)
            let lightOffColour = UIColor(hue: 0, saturation: 0, brightness: 0.34, alpha: 1)
            
            for row in stride(from: 10, to: Int(size.height - 10), by: 40) {
                for col in stride(from: 10, to: Int(size.width - 10), by: 40) {
                    if Bool.random() {
                        ctx.cgContext.setFillColor(lightOnColour.cgColor)
                    } else {
                        ctx.cgContext.setFillColor(lightOffColour.cgColor)
                    }
